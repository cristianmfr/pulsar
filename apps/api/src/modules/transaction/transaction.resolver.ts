...
export class TransactionResolver {
  ...
  findOne(@Args('id', { type: () => String }) id: string, @GetCurrentUser() user: CurrentUserType) {
    return this.transactionService.findOne(user.userId, id);
  }

  @UseGuards(AuthGuard)
  @Mutation(() => Transaction, { name: 'createTransaction' })
  create(@Args('data') data: CreateTransactionDto, @GetCurrentUser() user: CurrentUserType) {
