...
import { CompleteRegisterSchema, CompleteRegisterType } from '../../types/complete-register-step'
...
export const CompleteRegisterStep = () => {
  ...
  const [createUser, { loading }] = useMutation(CREATE_CUSTOMER)
  const [sendWelcomeEmail] = useMutation(CREATE_CUSTOMER)
  ...
  return (
    <form onSubmit={handleSubmit(handleFinishRegister)} className={cn('flex flex-col gap-6')}>
      <div className='flex flex-col items-center gap-2 text-center'>
        <span className='flex flex-row gap-2 text-2xl font-bold'>Finalize seu cadastro</span>
        <p className='text-balance text-xs text-muted-foreground'>Insira os dados da sua conta</p>
      </div>
      <RadioGroup defaultValue='card' className='grid grid-cols-3 gap-4'>
        ...
        <div>
          <RadioGroupItem value='pix' id='pix' className='peer sr-only' aria-label='Pix' />
          <Label
            htmlFor='pix'
            className='flex flex-col items-center justify-between rounded-md border-2 border-muted bg-transparent p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary'
          >
        ...
        <div>
          <RadioGroupItem value='boleto' id='boleto' className='peer sr-only' aria-label='Boleto' />
          <Label
            htmlFor='boleto'
            className='flex flex-col items-center justify-between rounded-md border-2 border-muted bg-transparent p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary '
          >
      ...
      <div className='flex flex-row gap-2'>
        <div className='grid gap-2'>
          <Input id='cpf' type='text' placeholder='Insira seu CPF' required {...register('cpf')} />
        </div>
        <div className='grid gap-2'>
          <Input id='phone' type='text' placeholder='Insira seu telefone' required {...register('phone')} />
        </div>
      </div>
      ...
      <div className='flex flex-col gap-2'>
        <div className='grid gap-2'>
          ...
          <Input id='code' type='text' placeholder='Insira seu código de afiliado' required {...register('code')} />
        </div>
        {loading ? (
          <Button disabled>
